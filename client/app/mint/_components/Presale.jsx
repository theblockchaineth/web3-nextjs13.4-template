...
export default function Presale() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await fetch('/api/getProofs', {
                next: { revalidate: 0 },
            })
    ...
    return (
        <section className="pb-8">
            <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
                ...
                <h1 className="flex items-center justify-center w-full h-16 bg-green-600 rounded-lg mb-8 font-marker text-xl tracking-widest">
                    <CheckCircleIcon height={24} width={24} className='mr-4' />
                    Allowlist Verified
                </h1>
                {/* Main 3 column grid */}
                            ...
                            <div className="overflow-hidden rounded-lg outline-dashed pb-8">
                                <div className="p-6 flex items-center justify-center text-center">
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        ...
                                        <PresaleMintButton contract={smartcontract} proof={proofs} decisions={mintItemArray} quantity={mintAmount} price={totalPriceRoundedAsString} />
                                    </div>
                                </div>
                                {proofs.length < 1 ? 
                                    (<p className='text-center mb-4 text-red-600 font-mono text-xs'>Merkle Proof Not Ready<XCircleIcon height={32} className={'text-center w-full my-2'} /></p>) 
                                        : 
                                    (<p className='text-center mb-4 text-green-600 font-mono text-xs'>Merkle Proof Ready<CheckCircleIcon height={32} className={'text-center w-full my-2'} /></p>)}
                            </div>
                        </section>
                    </div>
                </div>
